...
public:
	...
	void SetupScene();
	void SetupNeuronTree(NeuronTree nt, std::vector<float> &vertdata);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainApplication::Shutdown()//wwbmark shutdown vr
{
	if( m_pHMD )
	{
		vr::VR_Shutdown();
...
bool CMainApplication::HandleInput()
{
	...
	unControllerDeviceIndex =event.trackedDeviceIndex;
	uint32_t unAxisId;
	unAxisId = event.trackedDeviceIndex;
	unsigned short usDurationMicroSec=50;
	m_pHMD->TriggerHapticPulse(unControllerDeviceIndex,unAxisId,usDurationMicroSec );//*/
...
void CMainApplication::ProcessVREvent( const vr::VREvent_t & event )
{
	...
	{
		SDL_ShowSimpleMessageBox( SDL_MESSAGEBOX_ERROR, "Trigger pressed! ", "SUCCESS!", NULL );
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMainApplication::SetupTexturemaps()//wwbmark
{
	std::string sExecutableDirectory = Path_StripFilename( Path_GetExecutablePath() );
	std::string strFullPath = Path_MakeAbsolute( "./sphere.png", sExecutableDirectory );
...
void CMainApplication::SetupScene()
{
	...
	//SetupNeuronTree(loadedNT,vertdataarray);
	...
	//for( int z = 0; z< m_iSceneVolumeDepth; z++ )
	{
		//for( int y = 0; y< m_iSceneVolumeHeight; y++ )
		{
			//for( int x = 0; x< m_iSceneVolumeWidth; x++ )
			{
				...
				mat = mat * Matrix4().translate( 0, 5, 0 );
				AddCubeToScene( mat, vertdataarray );
	...
	glGenBuffers( 1, &m_glSceneVertBuffer );//vbo
...
//-----------------------------------------------------------------------------
// Purpose:set up each swc node vertex data of loaded NeuronTree  
//-----------------------------------------------------------------------------
void SetupNeuronTree(NeuronTree nt, std::vector<float> &vertdata)
{
	...
	for(int i = 0,k=0;i<SWC_num;i++)
	{
		...
		if(S0.pn==-1){
			//draw sphere
			//setupspherevertex(S0,vertdata)
		}
		else{
			//draw sphere
			//setupspherevertex(S0,vertdata);
			S1 = nt.listNeuron.at(S0.pn-1);
			//draw tube
			//setuptubevertex(S0,S1,vertdata);
		}
	}
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainApplication::AddCubeToScene( Matrix4 mat, std::vector<float> &vertdata )//wwbmark
{
	// Matrix4 mat( outermat.data() );

	Vector4 A = mat * Vector4( 0, 0, 0, 1 );//x,y,z,w
	Vector4 B = mat * Vector4( 1, 0, 0, 1 );//x+1,y,z,w
	Vector4 C = mat * Vector4( 1, 1, 0, 1 );//x+1,y+1
	Vector4 D = mat * Vector4( 0, 1, 0, 1 );//y+1
	Vector4 E = mat * Vector4( 0, 0, 1, 1 );//z+1
	Vector4 F = mat * Vector4( 1, 0, 1, 1 );//x+1,z+1
	Vector4 G = mat * Vector4( 1, 1, 1, 1 );//x+1,y+1,z+1
	Vector4 H = mat * Vector4( 0, 1, 1, 1 );//y+1,z+1
...
// Purpose:
//-----------------------------------------------------------------------------
void CMainApplication::SetupCameras()//wwbmark camera
{
